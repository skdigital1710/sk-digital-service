...
const emptyForm = {
  ...
  file: null, image: null, organizationLogo: null, organizationLogoUrl: "",
};
...
function Admin() {
  ...
  async function saveResource(e){
    ...
    try{
      ...
      if(form.file){if(form.file.type!=="application/pdf")throw new Error("Sirf PDF upload karein.");if(form.file.size>20*1024*1024)throw new Error("PDF maximum 20 MB ho sakti hai.");filePath=await uploadFile(form.file,"resources","notifications")}
      if(form.image){if(!form.image.type.startsWith("image/"))throw new Error("Thumbnail ke liye image file choose karein.");if(form.image.size>5*1024*1024)throw new Error("Image maximum 5 MB ho sakti hai.");imagePath=await uploadFile(form.image,"resource-thumbnails","jobs")}
      let organizationLogoPath=editingResource?.organization_logo_path||null;
      if(form.organizationLogo){if(!form.organizationLogo.type.startsWith("image/"))throw new Error("Organization logo ke liye image file choose karein.");if(form.organizationLogo.size>2*1024*1024)throw new Error("Organization logo maximum 2 MB ho sakta hai.");organizationLogoPath=await uploadFile(form.organizationLogo,"resource-thumbnails","organization-logos")}
  ...
  async function savePromotion(e){e.preventDefault();setSaving(true);setError("");try{let imagePath=editingPromotion?.image_path||null;if(promotion.image){imagePath=await uploadFile(promotion.image,"resource-thumbnails","promotions")}const payload={title:promotion.title.trim(),subtitle:promotion.subtitle.trim(),description:promotion.description.trim(),image_path:imagePath,image_url:imagePath?publicImageUrl(imagePath):promotion.imageUrl.trim(),button_text:promotion.buttonText.trim()||"View Now",button_url:promotion.buttonUrl.trim(),placement:promotion.placement,display_order:Number(promotion.displayOrder)||0,is_active:promotion.isActive,start_at:promotion.startAt?new Date(promotion.startAt).toISOString():null,end_at:promotion.endAt?new Date(promotion.endAt).toISOString():null,updated_at:new Date().toISOString()};let q=editingPromotion?supabase.from("promotions").update(payload).eq("id",editingPromotion.id):supabase.from("promotions").insert(payload);const {error:e1}=await q;if(e1)throw e1;setMessage("Promotion saved");setShowPromotionModal(false);loadAll()}catch(err){setError(err.message)}finally{setSaving(false)}}
